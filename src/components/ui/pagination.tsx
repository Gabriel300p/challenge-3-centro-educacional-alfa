...
export function Pagination<TData>({ table }: PaginationProps<TData>) {
  ...
  return (
    <div className="flex flex-col space-y-4 pt-4 lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:space-x-6">
      {/* Informações e seletor de items por página */}
      <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-3">
        <p className="text-sm font-medium text-slate-700">
          {totalItems} <span className="text-slate-400 font-normal">itens</span>
        </p>
        <hr className="hidden sm:block border h-6 border-slate-200" />
            ...
            {getVisiblePages().map((page, index) => {
              if (page === "...") {
                return (
                  <span
                    key={index}
                    className="px-2 text-slate-400 hidden sm:inline"
                  >
                    ...
                  </span>
                );
